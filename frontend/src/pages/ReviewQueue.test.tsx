...
/** Some history, nothing today and nothing under Review or Posted — the page stays on
 *  Today and paints its empty card, not `NoScansYet`. */
const QUIET = { ...ZERO, all: 2, unposted: 2 }
...
describe('the message column', () => {
  ...
  it('never paints a parked document in the colour of a dead one', async () => {
    ...
    mount([doc({ flags: ['mapping_missing'] })])
  ...
  it('shows only the most blocking reason, never a list', async () => {
    ...
    mount([doc({ flags: ['warnings', 'doc_no_missing', 'mapping_guessed', 'unbalanced'] })])
  ...
  it('says why the pipeline stopped, over anything it merely noticed', async () => {
    ...
    mount([doc({ reason_code: 'tax_id_mismatch', flags: ['unbalanced'] })])
  ...
  it('still offers Review on a row that stopped, because it is still postable', async () => {
    ...
    mount([doc({ reason_code: 'carmen_rejected' })])
...
describe('a skipped attachment on Not posted', () => {
  ...
  const onUnposted = async (rows: ReviewDocument[]) => {
    mount(rows, { ...ZERO, all: rows.length, unposted: rows.length }, rows.length)
...
describe('where a row came from', () => {
  ...
  it('shows a charged scan that never posted, red, with no way to resume it', async () => {
    // The row exists so a BU scanning by hand can see what each credit went on. Line
    // items are never persisted, so there is nothing to reopen — no button at all.
    mount([doc({ source: 'manual', status: 'scanned', total: 0, posted_by_name: 'somchai' })], {
      ...ZERO,
      all: 1,
      unposted: 1,
    })
    expect(await screen.findByText('Scanned by somchai — not posted')).toHaveClass('rq-reason--bad')
    const pill = screen.getAllByText('Not posted').find(el => el.classList.contains('rq-pill'))
    expect(pill).toHaveClass('rq-pill--bad')
    const row = screen.getByText('july.pdf').closest('tr')!
  ...
  it('falls back to "by hand" when the scanner can no longer be resolved', async () => {
    ...
    mount([doc({ source: 'manual', status: 'posted', jv_no: 'JV-7', total: 0 })])
    expect(await screen.findByText('Scanned and posted by hand')).toBeInTheDocument()
  })
...
describe('the JV number', () => {
  it('opens from the Actions column, not by clicking the number', async () => {
    ...
    mount([doc({ id: 'b', status: 'posted', jv_no: 'JV-7', total: 0 })])
  ...
  it('offers no button for a row posted before jv_no was recorded', async () => {
    ...
    mount([doc({ id: 'c', status: 'posted', jv_no: null, total: 0 })])
    await screen.findByText('KTC')
...
describe('the status filter chips', () => {
  it('offers three status chips between the day and the log, in reading order', async () => {
    ...
    mount([doc()], { all: 113, today: 4, review: 3, success: 7, unposted: 103 })
    await screen.findByText('KTC')
  ...
  it('opens on the day when the day has something in it', async () => {
    mount([doc()], { ...ZERO, all: 113, today: 4, review: 12, unposted: 101 })
    await screen.findByText('KTC')
  ...
  it('falls through a quiet day to the work owed', async () => {
    ...
    mount([doc()], { ...ZERO, all: 113, today: 0, review: 12, unposted: 101 })
    await waitFor(() => expect(vi.mocked(api.listActivity)).toHaveBeenCalledTimes(2))
  ...
  it('shows no empty table on the way through', async () => {
    ...
    mount([doc()], { ...ZERO, all: 113, today: 0, review: 12 })
    expect(screen.queryByText('No activity today')).not.toBeInTheDocument()
  ...
  it('asks the server for the whole log when All is clicked', async () => {
    mount([doc()], { all: 113, today: 5, review: 3, success: 7, unposted: 5 })
    await screen.findByText('KTC')
  ...
  it('refetches for the chip that was clicked', async () => {
    mount([doc()], { ...ZERO, all: 5, review: 1, success: 4 })
  ...
  it('does not re-land after a refresh', async () => {
    ...
    mount([doc()], { ...ZERO, all: 5, today: 5, review: 5 })
    await screen.findByText('KTC')
...
describe('the actions column', () => {
  ...
  it('offers nothing on a resolved row', async () => {
    ...
    mount([doc({ status: 'posted', jv_no: 'JV-9001', total: 0 })])
  ...
  it('gives a dead Carmen credential its own word, not a generic settings link', async () => {
    ...
    mount([doc({ status: 'failed', reason_code: 'carmen_unauthorized', total: 0 })])
    expect(
  ...
  it.each(['carmen_rejected', 'duplicate_document', 'unreadable_document'])(
    ...
    async reason_code => {
      mount([doc({ status: 'failed', reason_code, total: 0 })])
...
describe('a row that has already been resolved', () => {
  ...
  it('translates the reason a document did not post', async () => {
    mount([doc({ status: 'failed', reason_code: 'carmen_rejected', total: 0 })])
  ...
  it('does not dress a document we never finished as one we deliberately skipped', async () => {
    ...
    mount([doc({ status: 'received', reason_code: null, total: 0 })])
  ...
  it('falls back to the raw reason code rather than showing nothing', async () => {
    ...
    mount([doc({ status: 'failed', reason_code: 'something_new', total: 0 })])
  ...
  it('leaves a row with nothing to open inert', async () => {
    ...
    mount([doc({ status: 'failed', reason_code: 'carmen_rejected', jv_no: null })])
...
describe('the dot the whole business unit shares', () => {
  ...
  it('marks nothing when there is nothing wrong under the open chip', async () => {
    ...
    mount([doc()], { ...ZERO, all: 60, today: 1, review: 1, unposted: 59 }, 1, ZERO, {})
    await screen.findByText('KTC')
    fireEvent.click(screen.getByRole('tab', { name: /Not posted/ }))
    await waitFor(() => expect(vi.mocked(api.listActivity)).toHaveBeenCalledTimes(2))
