...
describe('the screen', () => {
  ...
  it('will not post without a prefix', async () => {
    // `Prefix` is sent as `config.file_prefix or ""`, so an unset one does not fail
    ...
    mount()
    expect(await screen.findByText(/Choose a prefix before posting/i)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Approve/ })).toBeDisabled()
  })

  it('posts once a prefix is chosen', async () => {
    storedConfig = { ...(storedConfig as object), filePrefix: 'JV' }
    vi.mocked(api.getPending).mockResolvedValue(detail())
    mount()
    await waitFor(() => expect(screen.getByRole('button', { name: /Approve/ })).toBeEnabled())
    expect(screen.queryByText(/Choose a prefix/i)).not.toBeInTheDocument()
  })
