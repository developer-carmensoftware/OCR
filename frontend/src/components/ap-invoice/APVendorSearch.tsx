...
export default function VendorSearch({ t, systemVendor, setSystemVendor, vendorSearch, setVendorSearch, showVendorDrop, setShowVendorDrop, filteredVendors, onRefresh, refreshing }: Props) {
  return (
    <div className="vendor-search-wrap">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <a href={getCarmenUrl('/apVendor/create')} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.25rem 0.65rem', fontSize: '0.75rem', fontWeight: 600, color: 'var(--primary)', background: 'var(--ap-exclude-bg, #eff6ff)', border: '1px solid var(--primary)', borderRadius: '999px', textDecoration: 'none', whiteSpace: 'nowrap' }}>
            <Plus size={11} /> New Vendor
          </a>
          <button type="button" onClick={onRefresh} disabled={refreshing} title="Refresh vendor list" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '1.75rem', height: '1.75rem', background: 'var(--gray-50)', border: '1px solid var(--border)', borderRadius: '999px', cursor: refreshing ? 'not-allowed' : 'pointer', color: 'var(--text-3)', opacity: refreshing ? 0.6 : 1 }}>
      ...
      <div className="vendor-search-input-wrap">
        ...
        <input
          ...
          onFocus={() => { if (systemVendor.code) setVendorSearch(''); setShowVendorDrop(true) }}
              ...
              return (
                ...
                >
                  <div className="vd-name" style={isInactive ? { color: 'var(--text-4)' } : undefined}>
                    {v.code} — {v.name}
                    {isInactive && <span style={{ marginLeft: '0.4rem', fontSize: '0.7rem', fontWeight: 600, color: 'var(--rose)', background: 'var(--btn-err-bg, #fee2e2)', borderRadius: '4px', padding: '0 4px' }}>Inactive</span>}
                  </div>
                  <div className="vd-meta">
                    <span className="vd-tax">Tax ID: {v.taxId}</span>
                    {v.branchNo != null && v.branchNo !== '' && <span className="vd-branch">BranchNo: {v.branchNo}</span>}
                  </div>
                </div>
              )
            })
          ) : (
            <div style={{ padding: '0.75rem 1rem', fontSize: '0.83rem', color: 'var(--text-4)', textAlign: 'center' }}>No vendor found</div>
          )}
        </div>
      )}
    </div>
