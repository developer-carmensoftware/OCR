...
export default function APSuccessStep({ t, headerData, lineItems, invoiceSeq, onReset }: Props) {
  const SUMMARY_ROWS = [
    { label: t.subTotal, val: fmt(headerData.subTotal) },
    { label: t.discount, val: fmt(headerData.totalDiscount), color: 'var(--rose)' as const },
  ...
  return (
    <div style={{ padding: '2rem 0' }}>
      <div className="ap-success-wrap">
        <div className="ap-success-icon"><CheckCircle2 size={48} /></div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
          {invoiceSeq && (
            <a href={getCarmenUrl(`/apInvoice/${invoiceSeq}/show`)} target="_blank" rel="noopener noreferrer" className="btn btn-outline">
