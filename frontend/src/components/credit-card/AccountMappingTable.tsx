...
import type { MainMappingKey, Suggestion, SuggestionSource } from '../../hooks/mapping/useMappingSuggestions'
    ...
    <div className="section">
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span>
            ...
            {loadingOpts && (
              <span style={{ marginLeft: '10px', fontSize: '0.8rem', color: 'var(--primary)', display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
      ...
      <div className="table-wrapper" style={{ paddingBottom: '0' }}>
        <div
          ...
          style={{ display: 'grid', gridTemplateColumns: '70px 140px minmax(0, 1fr) minmax(0, 1fr) auto', gap: '0.5rem', alignItems: 'center', width: '100%' }}
        >
          <div /><div />
          ...
          <div className="mapping-type type-credit" style={{ color: 'var(--primary)', background: 'var(--primary-light)', padding: '0.2rem 0.5rem', borderRadius: '4px', textAlign: 'center', fontWeight: 'bold' }}>
            Credit
          </div>
          <div className="mapping-label clickable" style={{ cursor: 'pointer', color: 'var(--primary)', textDecoration: 'underline' }} onClick={openAmountModal}>
            Account Receivable (Click to Map)
          </div>
          <div style={{ gridColumn: 'span 3' }}>
            <div
              id="amountMappingStatus"
              style={{ fontSize: '0.85rem', padding: '0.6rem 1rem', borderRadius: '8px', border: '1px solid', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0.5rem', color: requiredMissingCount > 0 ? '#dc2626' : 'var(--teal)', background: requiredMissingCount > 0 ? 'var(--btn-err-bg, #fff1f2)' : 'var(--teal-light)', borderColor: requiredMissingCount > 0 ? '#fca5a5' : 'var(--teal)' }}
            >
              {activeScan.paymentTypes.size === 0 ? (
                <><Info size={14} style={{ flexShrink: 0 }} /><span>Click <strong>Account Receivable</strong> to open mapping modal</span></>
                ...
                <>
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: '0.72rem', background: 'var(--rose)', color: 'white', padding: '2px 10px', borderRadius: '10px', fontWeight: 600, whiteSpace: 'nowrap', flexShrink: 0 }}>
          ...
          {(['commission', 'tax', 'net'] as MainMappingKey[]).map(key => {
            ...
            const badge = meta === 'history' ? { label: 'History', bg: '#f0fdf4', color: '#16a34a', border: '#86efac' } : null
            ...
            const deptTopChoice = suggestion?.dept ? { code: suggestion.dept, name: deptFromMaster?.name || '(AI/History code)', name2: deptFromMaster?.name2, source: suggestion.source } : null

            const accFromMaster = suggestion?.acc ? masterAccounts.find(a => a.code === suggestion.acc) : null
            const accTopChoice = suggestion?.acc ? { code: suggestion.acc, name: accFromMaster?.name || '(AI/History code)', name2: accFromMaster?.name2, source: suggestion.source } : null

            return (
              <React.Fragment key={key}>
                <div className="mapping-type type-debit" style={{ color: '#d97706', background: 'var(--ap-include-bg, #fef3c7)', padding: '0.2rem 0.5rem', borderRadius: '4px', textAlign: 'center', fontWeight: 'bold' }}>
                  Debit
                </div>
                ...
                <div style={{ display: 'flex', gap: '0.3rem' }}>
                  {hasSuggestionButtons && (
                    <>
                      <button type="button" onClick={() => confirmMainSuggestion(key)} title="Accept suggestion" style={{ padding: '4px 10px', background: '#f0fdf4', color: '#15803d', border: '1px solid #86efac', borderRadius: '6px', cursor: 'pointer', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: '0.3rem', fontWeight: 600 }}>
                        <Check size={13} />
                      </button>
                      <button type="button" onClick={() => rejectMainSuggestion(key)} title="Reject and clear" style={{ padding: '4px 10px', background: 'var(--btn-err-bg, #fff1f2)', color: 'var(--btn-err-text, #dc2626)', border: '1px solid var(--btn-err-border, #fecaca)', borderRadius: '6px', cursor: 'pointer', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: '0.3rem', fontWeight: 600 }}>
                        <X size={13} />
                      </button>
                    </>
                  )}
