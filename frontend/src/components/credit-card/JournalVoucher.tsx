...
export default function JournalVoucher({ jvRows, headerData, filePrefix, fileSource: _fileSource, description, carmenJvId, onFinish, onBack }: Props) {
  ...
  return (
    <>
      ...
      <Card
        ...
        right={<div style={{ display: 'flex', gap: '0.5rem' }}><Badge variant="gray">OCC</Badge><Badge variant="blue">Normal</Badge></div>}
      >
          ...
          <div className="table-wrapper">
            <table className="data-table jv-table">
              <thead>
                <tr>
                  {JV_HEADERS.map((h, i) => (
                    <th key={h} scope="col" className={i >= 6 ? 'text-right' : i === 2 || i === 3 ? 'text-left' : 'text-center'} style={{ whiteSpace: 'nowrap' }}>
                      {h}
                    </th>
                  ))}
                </tr>
        ...
        <div className="form-actions">
          {onBack && (
            <button type="button" className="btn-cancel" onClick={onBack} style={{ marginRight: 'auto' }}>
              <ArrowLeft size={14} /> Back
            </button>
          )}
          <button type="button" className="btn-cancel" onClick={handleOpenJv} disabled={!carmenJvId} title={carmenJvId ? `View JV #${carmenJvId}` : 'Waiting for JV ID'}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.35rem' }}>
            <span style={{ fontSize: '0.75rem', color: 'var(--text-4)', display: 'inline-flex', alignItems: 'center', gap: '0.3rem' }}>
              <ArrowRight size={12} /> Next Step: <strong>Input Tax Reconciliation</strong> — Verify Input Tax
            </span>
            <button type="button" className="btn-submit" onClick={() => { toast.info('Proceeding to Step 6: Input Tax Reconciliation'); onFinish() }}>
