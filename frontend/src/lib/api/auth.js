 ...
 * @param {string} user   - Username from URL params
 * @param {string} uri    - Carmen origin URI (window.location.origin of Carmen)
 * @returns {Promise<{ access_token: string, expires_in: number, user: object }>}
 */
export async function exchangeSSOToken(token, bu, user = '', uri = '') {
