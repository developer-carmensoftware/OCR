...
### `ingest_address` — ฟิลด์เดียวที่ลูกค้าต้องเห็น

ที่อยู่ที่ลูกค้าจะ forward เมลธนาคารมา **ไม่ซ้ำกันต่อ 1 BU** เอาไปแสดงให้ copy ได้เลย

```text
ocr+7f3a91@carmensoftware.com
        └── tag ต่อ BU เราจองให้ตอน PUT ครั้งแรก
```

| การรับประกัน | หมายความว่าอะไรกับ UI ฝั่ง Carmen |
|---|---|
| **ไม่มีวันเปลี่ยน** ตลอดอายุของ BU | cache ได้ แสดงซ้ำได้ ไม่ต้องกลัวลูกค้าตั้ง forward rule ไว้แล้วพัง |
| เป็น `null` จนกว่าจะ `PUT` ครั้งแรก | อ่านจาก response ของ `PUT` ได้เลย ไม่ต้อง `GET` ซ้ำ |
| ลูกค้าจะ forward อัตโนมัติหรือส่งมือทีละฉบับก็ได้ | ไม่ต้องมีสวิตช์แยก ระบบรองรับทั้งสองแบบและปนกันได้ |
