...
async def _check_cost_spike(target_date: date) -> int:
    ...
    async with async_session() as db:
        ...
        """), {"target": target_date, "base_start": baseline_start,
               "base_end": baseline_end, "mult": COST_SPIKE_MULTIPLIER})
        ...
        for r in rows.mappings().fetchall():
            ...
            if await _alert_exists(db, tid, bid, "cost_spike"):
                continue
            await _insert_alert(db, tenant_id=tid, business_unit_id=bid, module_id=mid,
                                metric="cost_spike", severity=AlertSeverity.WARN,
                                threshold=threshold, actual=actual,
                                description=(f"Daily LLM cost ${actual:.4f} exceeds "
...
async def _check_error_spike(target_date: date) -> int:
    ...
    async with async_session() as db:
        ...
        """), {"target": target_date, "base_start": baseline_start,
               "base_end": baseline_end, "min_count": ERROR_MIN_COUNT})
        ...
        for r in rows.mappings().fetchall():
            ...
            if await _alert_exists(db, tid, bid, "error_spike"):
                continue
            await _insert_alert(db, tenant_id=tid, business_unit_id=bid, module_id=None,
                                metric="error_spike", severity=AlertSeverity.WARN,
                                threshold=threshold, actual=today_rate,
                                description=(f"API error rate {today_rate:.1%} exceeds "
    ...
    async with async_session() as db:
        ...
        for r in rows.mappings().fetchall():
            ...
            if await _alert_exists(db, tid, None, metric):
                continue
            await _insert_alert(db, tenant_id=tid, business_unit_id=None,
                                module_id=None,
...
async def _insert_alert(db, *, tenant_id, business_unit_id, module_id,
                        metric, severity: AlertSeverity, threshold, actual, description) -> None:
    ...
    """), {
        "tid": tenant_id, "bid": business_unit_id, "mid": module_id,
        "metric": metric, "severity": severity.value,
        "threshold": float(threshold), "actual": float(actual), "desc": description,
    })
    logger.info("[anomaly] Alert: tenant=%s metric=%s severity=%s actual=%s",
                tenant_id, metric, severity.value, actual)
